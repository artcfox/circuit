...
uint8_t ConsultOracle(uint32_t nl)
{
  ...
  while (low <= high) {
    ...
    uint32_t netlist_and_led_state = (uint32_t)pgm_read_dword(&sorted_netlists_and_led_states[mid]);
    uint32_t netlist = netlist_and_led_state & NETLIST_NETLIST_MASK;
    ...
    if (netlist < nl) {
      ...
      uint8_t led_states = (uint8_t)((netlist_and_led_state & NETLIST_FLAG_MASK) >> 29);
      return led_states;
    }
  }
